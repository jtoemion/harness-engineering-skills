...
## ITERATIVE LEARNING LAYER (FULL MODE)

### Capture During Work
- `inefficiency: [brief description]` — log to `.memory/ill/captures.md`
- `efficiency win: [brief description]` — log to `.memory/ill/wins.md`
- Tags: `[delegation]` `[prompt-quality]` `[scope-creep]` `[cache]` `[memory]` `[subagent]` `[protocol]`
- Severity: `FRICTION` (minor) | `BLOCKER` (major)

### Synthesis (On-Command)
- Agent nudges at SESSION_CLOSE: "N captures since last synthesis"
- Trigger: `synthesize` command
- See: `{SKILLS_ROOT}\harness\ITERATIVE_LEARNING.md`

### Promotion
- Project pattern (3x) → seen in 2+ projects → GLOBAL_PATTERN
