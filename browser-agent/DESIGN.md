...
Browser automation should be backend-agnostic. The agent abstraction allows any CDP-connected backend (local browser, cloud browser, remote debug endpoint) to be swapped in without changing recipe logic.

```
Backend Interface (abstract)
├── connect()              → returns browser session
├── navigate(url)         → page load
├── click(selector)       → DOM or coordinate click
├── type(selector, text)  → input field typing
├── screenshot()          → current viewport
├── evaluate(script)      → arbitrary JS execution
└── wait_for(state)       → DOM assertion or URL change

Implementations:
├── Local CDP            → direct browser-harness connection
├── Cloud Browser        → e.g., BrowserStack, Playwright CDN
└── Remote Debug         → chrome-devtools debugging endpoint
```

Key implications:
- Recipes use abstract actions only — no backend-specific code
- Backend selection is a configuration choice at invocation time
- Failure context (screenshot, DOM snapshot) is backend-provided, not recipe-dependent
- `map-site.py` and `run-recipe.py` are backend-agnostic by design
