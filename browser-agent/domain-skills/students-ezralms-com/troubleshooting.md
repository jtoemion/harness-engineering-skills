...
## students.ezralms.com: The step to wait for the 'Username or Email' textb

### First seen
2026-04-23

### Recipe
Login with Jorah / 111111

### Failed step
{'step': 2, 'action': 'wait', 'target': 'textbox "Username or Email" visible', 'value': None}

### Symptom
The step to wait for the 'Username or Email' textbox to be visible failed, likely because the element is not recognized by that exact label or the DOM structure differs from the expectation.

### Fix
Try using a more flexible selector such as waiting for an input with attributes like input[name='username'], input[type='text'], or input[placeholder*='Username'], or wait for a more general login form container to be visible before interacting with the input fields.

### Confidence
medium

### Status
Auto-repair not yet verified
